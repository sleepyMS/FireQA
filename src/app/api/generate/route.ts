...
export async function POST(request: NextRequest) {
  ...
  // 에이전트 모드: SSE 대신 즉시 jobId 반환
  if (executionMode === "agent") {
    let template: Awaited<ReturnType<typeof prisma.qATemplate.findUnique>> = null;
    if (templateId) {
      template = await prisma.qATemplate.findUnique({ where: { id: templateId } });
    }
    const { jobId, parsedText, projectId: pid } = await createGenerationJob(file, projectInput, JobType.TEST_CASES, {
      userId: user.userId,
  ...
  return createSSEStream(async (writer) => {
    ...
    try {
      ...
      createNotification({
        ...
        title: "TC 생성이 완료되었습니다",
        linkUrl: `${process.env.NEXT_PUBLIC_APP_URL ?? ""}/generate/${jobId}`,
      });
